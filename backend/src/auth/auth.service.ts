...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<any> {
    ...
    const cognito_id = this.getCognitoId(
  ...
  async verifyToken(token: string): Promise<any> {
    try {
      ...
      if (error.message.includes('Invalid token format')) {
        throw new UnauthorizedException('Invalid token format');
      } else if (error.message.includes('Failed to fetch public key')) {
        throw new UnauthorizedException('Failed to fetch public key');
      }
  ...
  async syncUsers() {
    ...
    response.Users.forEach(async (cognitoUser) => {
      ...
      const name = cognitoUser.Attributes.find(
        (attr) => attr.Name === 'name',
      )?.Value;
